...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_weight(end(left(m))) & is_weight(end(right(m))):
        lrod = length(left(m)) * size(end(left(m)))
        rrod = length(right(m)) * size(end(right(m)))
        if lrod == rrod:
            return True
        return  False
    lrod = length(left(m)) * total_weight(end(left(m)))
    rrod = length(right(m)) * total_weight(end(right(m)))
    if lrod == rrod:
        if is_mobile(end(left(m))):
            Lm = balanced(end(left(m)))
        else:
            Lm = True
        if is_mobile(end(right(m))):
            Rm = balanced(end(right(m)))
        else:
            Rm = True
        if Lm & Rm:
            return True
